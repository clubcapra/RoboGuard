...
bq769x0::bq769x0(TwoWire *theWire, byte bqType, int bqI2CAddress)
{
  type = bqType;
...
int bq769x0::begin(byte alertPin, byte bootPin)
{
  ...
  for (byte i = 0; i < NUMBER_OF_CELLS_INPUT; i++)
  {
    cellReadingVoltages[i] = 0;
  }
...
void bq769x0::updateTemperatures()
{
  ...
  int vtsx = 0;
  unsigned long rts = 0;
  uint8_t errorStatusI2c = 0;

  Wire->beginTransmission(I2CAddress);
  Wire->write(0x2C);
  errorStatusI2c = Wire->endTransmission();
...
void bq769x0::updateVoltages()
{
  ...
  errorStatusI2c = Wire->endTransmission(); // end transmission so that read can begin
  ...
  if (errorStatusI2c == 0)
  {
    ...
    int j = 0;
    for (int i = 0; i < NUMBER_OF_CELLS_INPUT; i++)
    {
      bool bCellShort = false;
      if (crcEnabled == true)
      {
      ...
      cellReadingVoltages[i] = adcVal * adcGain / 1000 + adcOffset; // calculate real voltage in mV

      for (int k = 0; k < N_BATTERY_CELLS_SHORT; k++)
      {
        if (i == (cellShort[k] - 1))
        {
          bCellShort = true;
      ...
      if (!bCellShort)
      {
        cellVoltages[j] = cellReadingVoltages[i]; // copy the reading to the final array

        // filter out voltage readings from unconnected cell(s)
        if (cellVoltages[j] > 500)
        {
          connectedCells++; // add one to the temporary cell counter var - only readings above 500mV are counted towards real cell count
        }

        if (cellVoltages[j] > cellVoltages[idCellMaxVoltage])
        {
          idCellMaxVoltage = i;
        }

        if (cellVoltages[j] < cellVoltages[idCellMinVoltage] && cellVoltages[j] > 500)
        {
          idCellMinVoltage = j;
        }
        j++;
...
uint8_t bq769x0::writeRegister(byte address, int data)
{
  LOG_PRINT("write: ");
